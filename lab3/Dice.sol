...
contract Dice {
    ...
    struct dice {
        ...
        // dicestate is only stationary or rolling
        diceState state;
        // when we create dice, we use ether 
        // so it stores that here
    ...
    event luckyTimesEvent (uint256 diceId, uint256 luckyTimes);
    
    // data structures to store the events
    uint256 public numDices = 0;
    mapping(uint256 => dice) public dices;

    //function to create a new dice
    // and add to 'dices' map. requires at least 0.01ETH to create
    function add(
        uint8 numberOfSides,
        uint8 color
    ...
    modifier validDiceId(uint256 diceId) {
        require(diceId < numDices, "This is not a valid dice id");
        _;
    }
    ...
    // destroy dice
    // return ether back to owner
    // check valid id and only owner can destroy
    function destroyDice(uint256 diceId) public ownerOnly(diceId) validDiceId(diceId) {
        uint256 value = dices[diceId].creationValue;
        delete dices[diceId]; // remove from dicelist
        // send value back to the sender
        msg.sender.transfer(value);

        // selfdestruct(); // THIS SELF DESTRUCTS THE CONTRACT
    }
